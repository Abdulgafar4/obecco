...
(function () {
  ...
  function heroField(canvas, opts) {
    ...
    function draw() {
      ...
      var scale = Math.min(w * 0.42, h * 0.62) * (opts.scale || 1);
      ...
      for (i = 0; i < edges.length; i++) {
        ...
        var dep = (A.z + B.z) / 2;
        var al = 0.26 * edges[i].w * (1 - dep * 0.9) * fade;
      ...
      for (i = 0; i < pulses.length; i++) {
        ...
        ctx.fillStyle = 'rgba(' + accent + ',' + (0.9 * fade).toFixed(3) + ')';
        ctx.fillRect(px - 2, py - 2, 4, 4);
      }
      for (i = 0; i < N; i++) {
        var p = P[i];
        var sq = (p.hub ? 5.4 : 2.8) * p.s;
        var al2 = (p.hub ? 0.95 : 0.46) * (1 - p.z * 0.7) * fade;
